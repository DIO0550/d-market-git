...
全項目スキップ可能（スキップ時はデフォルト値を使用）。

1. **デフォルト優先度**: `priority:P1` / `priority:P2` / `priority:P3`（デフォルト: P2）
2. **デフォルトサイズ**: `size:S` / `size:M` / `size:L`（デフォルト: M）
3. **カスタムラベル**: 全Issueに自動付与したいラベル（例: `sprint:2026-Q1`）
4. **担当者**: デフォルトのassignee、またはエリア別担当者
5. **マイルストーン**: 設定するマイルストーン名
6. **GitHub Project**: 紐付けるProject名
