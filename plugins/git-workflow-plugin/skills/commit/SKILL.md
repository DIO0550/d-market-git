...
本文は **変更内容の要約** で始める。subject 直後に **何をしたか** を箇条書きで書き、`git log` だけで変更の全体像を掴めるようにする。この要約に見出しは付けない（subject 直後の箇条書きは慣習的に変更の要約であり、見出しは冗長になるため）。diff の逐語訳ではなく、変更の趣旨がわかる粒度でよい。
