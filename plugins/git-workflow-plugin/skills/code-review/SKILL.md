---
name: code-review
description: コードレビュースキル。ブランチ名またはPR番号を指定して、差分コードのレビューを実施しGitHubにレビューコメントを投稿する。「レビューして」「コードレビュー」「PRをレビュー」「ブランチのレビュー」などのリクエスト時に使用。プロジェクト規約に基づいた総合的なコードレビューを行う。
...
ブランチ名またはPR番号から差分を取得し、プロジェクト規約に基づいた総合的なコードレビューを行い、GitHubにレビューコメントを投稿するスキル。

## ワークフロー

```
1. 引数の解釈（PR番号 or ブランチ名の判定）
   ↓
2. diff取得（変更ファイル一覧と差分内容）
   ↓
3. プロジェクト規約の収集（CLAUDE.md、lint設定、tsconfig等）
   ↓
4. 変更ファイルの全文読み込み（差分だけでなくコンテキストを理解）
   ↓
5. レビュー観点ごとに分析
   ↓
6. レビューコメント作成・投稿（gh pr review）
```
...
| 入力 | 判定 | 処理 |
|:-|:-|:-|
| 数値のみ（例: `123`） | PR番号 | `gh pr view 123` で存在確認 |
| `#123` 形式 | PR番号 | `#`を除去してPR番号として処理 |
| 文字列（例: `feature/add-login`） | ブランチ名 | ブランチに紐づくPRを`gh pr list --head {branch}`で検索 |

### ブランチ名からPRが見つからない場合

ブランチ名を指定されたがPRが見つからない場合は、ユーザーに報告して処理を中断する。レビューコメントの投稿先PRが必要なため、PR未作成の状態ではレビューを実行しない。
...
GitHubの「Files changed」上で、レビュアーが各変更行にコメントを付けて最後にまとめてSubmitする、あの通常のレビューフローと同じ形式で投稿する。
