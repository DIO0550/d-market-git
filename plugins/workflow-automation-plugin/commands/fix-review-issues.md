...
1. PR のレビューコメントのうち、コメント自体が未解決状態（resolved されていない）のもののみを `gh api graphql` で取得して確認する（スレッドIDも記録しておく）
...
3. 各指摘事項について：
   ...
   - コミット後、`gh api graphql` の `resolveReviewThread` mutation で該当スレッドを解決済みにする
